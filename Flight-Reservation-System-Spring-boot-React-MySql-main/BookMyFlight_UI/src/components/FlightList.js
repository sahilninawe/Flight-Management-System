...
import { withRouter } from 'react-router-dom'; //access to history object, navigate to different pages

class FlightList extends React.Component {
    constructor(props) {
        super(props);
        this.state = { //define the components state, state is an object that can change over time such as user i
            flights : null
        }
    }

    componentDidMount() {// this method is called ather the component has been mounted to the dom
        this.setState({
     ...
     */
      calculateDuration = (f) => {
        let t1 = new Date('1970-01-01T' + f.departureTime + 'Z')//Zis the timezone that indicate the UTC(Coordinated Universal Time (UTC).)
